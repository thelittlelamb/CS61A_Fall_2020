...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        def damage_place_bees(place, amount):
        ...
        damage_place = self.place # copy还是引用？
        Ant.reduce_armor(self, amount) # 虽然override了，但是还是可以使用class method
        damage_place_bees(damage_place, amount)

        if self.place is None: # if dead
            damage_place_bees(damage_place, self.damage)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        if bee:
            bee.reduce_armor(bee.armor) # 扣除全部的armor而不是直接remove
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        self.is_true_queen = QueenAnt.is_true_queen # class attribute
    ...
    def action(self, gamestate):
        ...
        if self.is_true_queen:
            ...
            ants_behind = []
            p = self.place.exit
            while p is not None: # The exit of a Place at the end of a tunnel is None.
                ants_behind.append(p.ant)
                p = p.exit
            
            for ant in ants_behind:
                if ant and not ant.double and ant.damage:
                    ant.damage *= 2
                    ant.double = True
        else:
